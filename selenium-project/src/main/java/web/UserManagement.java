...
public class UserManagement {
	...
	public static void main(String[] args) throws Exception {
		...
		WebElement mobileField = wait.until(
				ExpectedConditions.elementToBeClickable(By.cssSelector("input.crm__custom__mobile__input__field")));
		mobileField.click();
		mobileField.clear();
		...
		selectDropdownOption(driver, wait, js, By.id("authentication_type_select"), "Login User");
		System.out.println("✅ Console Access: Login User selected!");

		// ── STEP 6: User Type → Exicom ───────────────────────────────────────
		selectDropdownOption(driver, wait, js, By.id("user_type_select"), "Exicom");
		...
		// ── STEP 8: Pin Code ─────────────────────────────────────────────────
		WebElement pinDropdown = scrollIntoView(wait, js, By.id("site__pinCode_dropdown"));
		pinDropdown.click();

		wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='site__pinCode_dropdown']//input")))
				.sendKeys("411039");

		wait.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//div[contains(@class,'crm__dropdown__option')]"))).click();
		System.out.println("✅ Pincode 411039 selected!");

		// ── STEP 9: Address ──────────────────────────────────────────────────
		clearAndType(wait, js, By.id("user_addressLine1"), "123 Test Street, Test Area");
		...
		// ── STEP 10: Role ────────────────────────────────────────────────────
		selectDropdownOption(driver, wait, js, By.id("user_role_select"), "Account Coordinator");
		System.out.println("✅ Role: Account Coordinator selected!");

		// ── STEP 11: Circle ──────────────────────────────────────────────────
		selectDropdownOption(driver, wait, js, By.id("user_circle_select"), "Maharashtra");
		System.out.println("✅ Circle: Maharashtra selected!");

		// ── STEP 12: Product Selection ----
		selectMultiDropdown(wait, js, 8, "AC Charger");
		System.out.println("✅ Product selected: AC Charger");

		selectMultiDropdown(wait, js, 8, "DC Charger");
		System.out.println("✅ Product selected: DC Charger");

		// ---- STEP 13: Service Selection -----
		selectMultiDropdown(wait, js, 9, "Task");
		System.out.println("✅ Service selected: Task");

		selectMultiDropdown(wait, js, 9, "Ticket");
		System.out.println("✅ Service selected: Ticket");

		// ── STEP 12: Submit ──────────────────────────────────────────────────
		clickById(wait, js, "update-user");
		validateToast(wait, "successfully", "User Created", "User Creation");

		// ── SEARCH BY MOBILE & VALIDATE ──────────────────────────────────────
		System.out.println("\n--- Searching User by Mobile ---");
		clickByText(wait, "User Management");

		WebElement searchBox = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("users__search-box")));
		searchBox.clear();
		...
		WebElement userNameLabel = wait.until(ExpectedConditions.visibilityOfElementLocated(
				By.xpath("//p[contains(@id,'labeluser_name')" + " and contains(text(),'AutoTest LNameuser')]")));
		...
		Thread.sleep(2000);
		System.out.println("\n--- Updating Address ---");
		clearAndType(wait, js, By.id("user_addressLine1"), "456 Updated Street, New Area");
		System.out.println("✅ Address updated!");

		clickById(wait, js, "update-user");
		validateToast(wait, "successfully updated", "User Updated", "User Update");
		...
		By searchLocator = By.id("users__search-box");

		WebElement deleteVerifySearch = wait.until(ExpectedConditions.elementToBeClickable(searchLocator));

		js.executeScript("arguments[0].scrollIntoView({behavior:'smooth',block:'center'});", deleteVerifySearch);
		...
		// Clear safely
		deleteVerifySearch.sendKeys(Keys.chord(Keys.COMMAND, "a"));
		deleteVerifySearch.sendKeys(Keys.DELETE);
		...
		// IMPORTANT: RE-FETCH ELEMENT AGAIN
		deleteVerifySearch = wait.until(ExpectedConditions.elementToBeClickable(searchLocator));
		...
		Thread.sleep(2000);
		WebElement userByEmail = wait.until(ExpectedConditions.visibilityOfElementLocated(
				By.xpath("//p[@id='labeluser_name' and normalize-space()='AutoTest LNameuser']")));
		...
		WebElement dialogText = wait
				.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("div.dialog-info")));
		...
		WebElement confirmYesBtn = wait
				.until(ExpectedConditions.elementToBeClickable(By.id("delete-user-confirmation")));
		js.executeScript("arguments[0].scrollIntoView({behavior:'smooth',block:'center'});", confirmYesBtn);
		...
		validateToast(wait, "successfully deleted", "User Deleted", "User Deletion");
		...
		By searchBoxLocator = By.id("users__search-box");

		// Fetch fresh element
		WebElement deleteVerifySearch1 = wait.until(ExpectedConditions.elementToBeClickable(searchBoxLocator));

		js.executeScript("arguments[0].scrollIntoView({behavior:'smooth',block:'center'});", deleteVerifySearch1);

		deleteVerifySearch1.click();

		Thread.sleep(500);

		// Clear existing value safely
		deleteVerifySearch1.sendKeys(Keys.chord(Keys.COMMAND, "a"));
		deleteVerifySearch1.sendKeys(Keys.DELETE);
		...
		// IMPORTANT → re-fetch element after React re-render
		deleteVerifySearch1 = wait.until(ExpectedConditions.elementToBeClickable(searchBoxLocator));

		// Enter email
		deleteVerifySearch1.sendKeys(email);

		System.out.println("🔍 Searched email post-deletion: " + email);

		WebElement noDataEl = wait.until(
				ExpectedConditions.visibilityOfElementLocated(By.cssSelector("div.crm__table__dashboard__no__data")));
		...
		if (noDataText.contains("No Users Found")) {
			...
			driver.quit();
			throw new RuntimeException("❌ TEST FAILED: Deleted user still appears in search! Text: " + noDataText);
		}
		 ...
		 * 
		 * By toggleLocator =
		 * By.cssSelector("input.PrivateSwitchBase-input.MuiSwitch-input");
		 * 
		 ...
		 * 
		 * System.out.println("\n--- Clicking Update after toggle ---"); clickById(wait,
		 * js, "update-user"); validateToast(wait, "successfully updated",
		 ...
		 * By.xpath("//*[@id='crm__filter__value__dropdown']/div")) ); js.executeScript(
		 * "arguments[0].scrollIntoView({behavior:'smooth',block:'center'});",
		 * filterValueDropdown); filterValueDropdown.click();
		 * System.out.println("✅ Filter value dropdown clicked!");
		 * 
		 * WebElement inactiveChip = wait.until(
		 * ExpectedConditions.elementToBeClickable(
		 * By.xpath("//div[@aria-label='Inactive']")) ); inactiveChip.click();
		 ...
		 * " and contains(text(),'User Management')]")) ); pageHeader.click();
		 ...
		 * mobileSearch.clear(); mobileSearch.sendKeys(randomMobile);
		 * System.out.println("🔍 Searched mobile: " + randomMobile);
		 * 
		 * WebElement inactiveStatusEl; try { inactiveStatusEl = wait.until(
		 * ExpectedConditions.visibilityOfElementLocated(
		 ...
		 * statusInTable); driver.quit(); throw new RuntimeException(
		 * "❌ TEST FAILED: User status is not Inactive! Found: " + statusInTable); } }
		 * catch (TimeoutException e) { System.out.
		 * println("❌ FINAL VALIDATION FAILED – Inactive status badge not found!");
		 * driver.quit(); throw new RuntimeException(
	...
	private static void selectMultiDropdown(WebDriverWait wait, JavascriptExecutor js, int index, String value)
			throws InterruptedException {
		WebElement dropdown = wait.until(ExpectedConditions.elementToBeClickable(
				By.xpath("(//div[contains(@class,'crm__dropdown__input-container')])[" + index + "]//input")));
	...
	/**
	 * Reads the selected value of a CRM custom dropdown. Falls back to manual
	 * selection if nothing is pre-filled.
	 */
	private static String getDropdownValue(WebDriver driver, WebDriverWait wait, JavascriptExecutor js,
			String dropdownId, String label) throws InterruptedException {
		String value = "";
		try {
			value = driver.findElement(By
					.xpath("//div[@id='" + dropdownId + "']" + "//div[contains(@class,'crm__dropdown__single-value')]"))
					.getText().trim();
			...
			js.executeScript("arguments[0].scrollIntoView({behavior:'smooth',block:'center'});", dd);
			dd.click();
			Thread.sleep(500);
			WebElement option = wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath("//div[contains(@class,'crm__dropdown__option')]")));
